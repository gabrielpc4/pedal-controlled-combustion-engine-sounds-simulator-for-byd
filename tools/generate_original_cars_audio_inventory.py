...
def resolve_event_paths(
    ...
    # Some official GUID exports preserve mixed-case car folder names while
    # the installation directory is lowercase. Treat the folder component as
    # case-insensitive so an otherwise exact authored GUID is not rejected.
    ...
    for event in events:
        ...
        candidates = tuple(path for path in guid_paths.get(guid, ()) if path.casefold().startswith(normalized_prefix))
        if len(candidates) != 1:
            raise InventoryError(
...
def compile_inventory(
    ...
    *,
    ...
    if not original_sources:
        raise InventoryError("no usable original Assetto Corsa profiles were discovered")
...
def render_markdown(inventory: Mapping[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            f"This file is generated by `tools/generate_original_cars_audio_inventory.py`. It inventories all {len(inventory['cars'])} usable original Assetto Corsa profiles discovered from the installation, including Nissan Skyline GT-R R34 where present. It deliberately excludes the modded profiles.",
            "",
            ...
            f"- Every original profile has exactly one `engine_int` and one `engine_ext` ({inventory['summary']['engineIntProfileCount']}/{len(inventory['cars'])} and {inventory['summary']['engineExtProfileCount']}/{len(inventory['cars'])} confirmed).",
            f"- {inventory['summary']['transmissionProfileCount']}/{len(inventory['cars'])} profiles author `transmission`; {inventory['summary']['transmissionExteriorProfileCount']}/{len(inventory['cars'])} author `transmission_ext`. The current catalog therefore uses the native `transmission` fallback for exterior listening whenever transmission exists.",
