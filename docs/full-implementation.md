...
8. The independent sound RPM behavior follows the **P / N / D** shifter: **D** integrates an editable positive pedal force and independent lift-off/brake forces; road speed only multiplies positive force by the normalized Seal power envelope. **N** and **P** free-rev toward a throttle-position target using fixed neutral inertia constants (`NEUTRAL_REV_UP_RESPONSE_SECONDS` = 0.55 s, `NEUTRAL_REV_DOWN_RESPONSE_SECONDS` = 0.90 s).
