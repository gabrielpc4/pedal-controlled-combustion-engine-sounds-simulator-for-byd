...
8. The independent sound RPM target follows the **P / N / D** shifter: **D** integrates throttle-driven RPM scaled by the wheel-power curve, with lift-off fall at the configured coast rate (editable **DRIVE RPM** sliders); **N** and **P** free-rev with filtered throttle using fixed neutral inertia constants (`NEUTRAL_REV_UP_RESPONSE_SECONDS` = 0.55 s, `NEUTRAL_REV_DOWN_RESPONSE_SECONDS` = 0.90 s). During presentation shifts in **D**, RPM still blends toward the ratio-derived target with `syntheticRpmResponseSeconds` (default 35 ms).
