...
| Chevrolet Corvette C6 Z06 Stanced | `engine_ext` is silent at the tested listener positions; the interior route has a delayed source-bank activation. | Native runtime uses `engine_int` as its exterior fallback and preserves the authored event rather than replacing cells. | Recover an authored external route only if a later source bank supplies one. |
| Chevrolet Corvette C7 Stingray HellSpec | Same C6-derived delayed interior activation and silent `engine_ext` behavior. | Native runtime uses `engine_int` as its exterior fallback and preserves the authored event rather than replacing cells. | Recover an authored external route only if a later source bank supplies one. |
| Aston Martin DBS (DBRS9 GT3 bank) | `engine_ext` is silent while `engine_int` is audible. | The exterior toggle uses the same bank's audible interior event. | Recover an authored external route if the source package is updated. |
